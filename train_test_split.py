...
def split_dataset(img_folder, label_folder, train_ratio=0.7, val_ratio=0.15, test_ratio=0.15):
    ...
    for file_name in train_set:
        ...
        shutil.move(label_path, os.path.join(train_folder, file_name.replace('.png', '.txt')))

    for file_name in val_set:
        img_path = os.path.join(img_folder, file_name)
        label_path = os.path.join(label_folder, file_name.replace('.png', '.txt'))
        shutil.move(img_path, os.path.join(val_folder, file_name))
        shutil.move(label_path, os.path.join(val_folder, file_name.replace('.png', '.txt')))

    for file_name in test_set:
        img_path = os.path.join(img_folder, file_name)
        label_path = os.path.join(label_folder, file_name.replace('.png', '.txt'))
        shutil.move(img_path, os.path.join(test_folder, file_name))
        shutil.move(label_path, os.path.join(test_folder, file_name.replace('.png', '.txt')))

# Set your paths
img_folder = '/home/bapary/Videos/koala/images/'
label_folder = '/home/bapary/Videos/koala/labels/'
